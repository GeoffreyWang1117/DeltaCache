"""Llama-2-7B-only re-run of MoE-nD vs LayerBudget head-to-head.

Original run failed OOM on GPU 0 due to external process. This script forces
GPU 1 and uses expandable_segments to be safe.
"""
...
def main():
    ...
    out_path = OUT_DIR / f"moend_vs_lb_llama2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
