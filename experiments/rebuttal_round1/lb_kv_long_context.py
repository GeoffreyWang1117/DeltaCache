"""LayerBudget-KV at 16K context on Mistral-7B WikiText-2 PPL.

Compares LayerBudget-KV against LayerBudget at 16K to extend §4.3's long-context
table to the LB-KV variant. Mistral-7B is the model with the most pronounced
extreme-CR cliff for original LayerBudget (1.250 at CR=6× on 1024 tokens), so
this is the natural target for extending the long-context characterization.

CRs: 2× (where LB wins by 3pp at short context) and 4× (regime crossover point).
Reduced to seq=4096 instead of 16384 if budget is tight.
"""
...
SEQ_LEN = 2048  # Eager-attention at 4K OOMs on shared RTX 3090; 2K is 2× the baseline head-to-head.
N_CHUNKS = 4
PREFIX_RATIO = 0.6
...
def main():
    ...
    print(f"Loading {HF_NAME} at seq_len={SEQ_LEN} ...", flush=True)
    ...
    out_path = OUT_DIR / f"lb_kv_long_context_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
